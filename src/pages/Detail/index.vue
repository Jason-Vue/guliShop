      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr) in spuSaleAttrList"
                ...
                <dd changepirce="0"
                    ...
                    @click="changeChecked(spuSaleAttr,index)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 输入框的blur事件，是失去焦点事件，只要添加了它，失去焦点就会触发这个事件
                但是不好，因为它不会去判断这次输入的数据和之前的数据是不是一样再触发
...
export default {
  ...
  computed: {
    ...
    // 2.给子组件zoom和ImageList的数据
    skuImageList () {
      // 如果数据没有回来，skuInfo是个空对象,this.skuInfo.skuImageList就是undefined
      return this.skuInfo.skuImageList || []
    },

  },
  methods: {
    // 1.改变售卖属性的选中状态(使用排他思想)
    changeChecked (spuSaleAttr, index) {
      // console.log(spuSaleAttr, index);
      // 数组里面的选项isChecked都不选中
      spuSaleAttr.spuSaleAttrValueList.forEach((item, index) => {
